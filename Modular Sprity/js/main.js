...
let imageURL = "https://www.pngkey.com/png/full/54-540376_tropical-flower-transparent-background.png";
...
function init() {
    ...
    utils.preloadImage(imageURL, function(image) {
        spriteImage = image;
        sprites = sprites.concat(sprites, createImageSprites(15, spriteImage, classes.ImageSprite));
...
function createImageSprites(num, image, classRef = classes.ImageSprite) {
    let imgSpriteArray = [];
    ...
    for (let i = 0; i < num; i++) {
        ...
        let fwd = utils.getRandomUnitVector();
        let speed = Math.random() + 2;
        let rotSpeed = Math.PI / (Math.random(1,6));
        let s = new classRef(x, y, span, fwd, speed, rotSpeed, image);

        // add to end of array
        imgSpriteArray.push(s);
    }

    return imgSpriteArray;
}
...
function rotateImageSprites(ctx){
    ctx.save();
    for (let i = 30; i < 45; i++){
        ctx.rotate(sprites[i].rotSpeed);
        sprites[i].draw(ctx);
    }
